...
def _read_mp_lookup(shopee_bytes, tiktok_bytes_list, lazada_bytes):
    ...
    ordered_date, nickname, mp_sla, item_name, seller_sku}
    ...
    # TikTok — only Order ID available, fill rest as blanks
            ...
            for _, r in laz.iterrows():
                ...
                if oid not in lookup:
                    lookup[oid] = {
                        ...
                        "nickname":          "lazada-SG",
                        "mp_sla":            sla if sla and sla != "nan" else "",
                ...
                if do_update:
                    ...
                    with st.spinner(f"Updating {inv_num}…"):
                        try:
                            ...
                            updated_any_np = True
                            st.success(f"✅ `{inv_num}` → FILTERED DATA with `{new_wms}`. PIVOT rebuilt.")
                            st.download_button(
    ...
    if nitc_rows:
        ...
        for idx, row in enumerate(nitc_rows):
            ...
            with st.expander(
                ...
                f"|  {row.get('Store','—')}  |  {row.get('Item Name', row.get('item_name','—'))[:40]}",
                expanded=False,
            ):
                # Show what we already know from MP file
                c_info, c_inputs, c_btn = st.columns([2, 2, 1])
                with c_info:
                    st.markdown(f"**Order Number:** `{order_num}`")
                    st.markdown(f"**Marketplace:** {row.get('Marketplace','')}")
                    st.markdown(f"**MP Status:** {row.get('MP Status','')}")
                    st.markdown(f"**Ordered Date:** {mp_info.get('ordered_date', row.get('Ordered Date','—'))}")
                    st.markdown(f"**MP SLA:** {mp_info.get('mp_sla', row.get('MP SLA','—'))}")
                    st.markdown(f"**Nickname:** {mp_info.get('nickname', row.get('Store','—'))}")
                with c_inputs:
                ...
                if do_nitc_update:
                    if not inv_input.strip():
                        ...
                        # Build full row from MP lookup + manual inputs
                        new_row = {
                            ...
                            "Ordered Date":      mp_info.get("ordered_date", row.get("Ordered Date", "")),
                            "Nickname":          mp_info.get("nickname", row.get("Store", "")),
                            "MP SLA":            mp_info.get("mp_sla", row.get("MP SLA", "")),
                            "WMS":               wms_input,
                        }
                        with st.spinner(f"Updating {order_num}…"):
                            try:
                                ...
                                st.session_state["np_wms_rows"] = np_rows2
                                st.session_state["nitc_rows"]   = nitc_rows2
                                updated_any_nitc = True
                                st.success(
                                    f"✅ `{order_num}` → FILTERED DATA  "
                                    f"(Invoice: `{inv_input.strip()}`, WMS: `{wms_input}`). PIVOT rebuilt."
                                )
                                st.download_button(
                                    "⬇️ Download Updated Report",
                                    data=updated_bytes,
