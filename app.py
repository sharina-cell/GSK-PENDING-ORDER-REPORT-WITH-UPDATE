...
PARTIAL_COLS   = ["Invoice Number", "Payment Status", "Order Item Status",
                  "Ordered Date", "Nickname", "MP SLA", "WMS",
                  "WMS Custom SKU", "Item Description",
                  "Qty Ordered", "Qty Allocated", "WH Stock", "Remarks"]
PARTIAL_WIDTHS = [22, 16, 22, 22, 30, 14, 24, 28, 35, 12, 14, 12, 12]
PARTIAL_ORDER_COLS = 7   # first 7 cols are order-level (blanked on sub-rows)
PARTIAL_WMS_COL   = 7   # 1-based index of WMS column in PARTIAL sheet
...
def generate_report(tc_file, shopee_file, wms_files, inv_file):
    ...
    if "client_order_id" in wms.columns:
        for _, wr in wms.iterrows():
            ...
            if oid:
                wms_lines_by_inv[oid].append(wr)
    ...
    def get_remarks(qty_alloc, wh_stock):
        ...
        - qty_alloc >= 1          → FULFILL
        - qty_alloc == 0, stock 0 → CANCEL
        - qty_alloc == 0, stock>0 → FULFILL
        - nulls                   → blank
        """
        if qty_alloc is not None and qty_alloc >= 1:
            return "FULFILL"
        if qty_alloc == 0 and wh_stock is not None:
    ...
    for i, (_, r) in enumerate(df_partial.iterrows(), 1):
        ...
        if wms_lines:
            for line_idx, wd in enumerate(wms_lines):
                ...
                if is_first:
                    order_vals = [r["invoice_number"], r["payment_status"],
                ...
                vals = order_vals + [
                    ...
                    qty_alloc   if qty_alloc   is not None else "",
            ...
            vals = [r["invoice_number"], r["payment_status"],
                    r["order_item_status"], r["ordered_date"],
                    r["nickname"], r["MP SLA"], r["WMS"],
                    wms_sku, "", "", "",
