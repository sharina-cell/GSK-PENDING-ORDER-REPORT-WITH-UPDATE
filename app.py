...
def build_pivot_sheet(ws, filtered_rows, headers, today_str):
    ...
    # Clear previous content — use generous ceiling to eliminate ghost rows/cols
    old_r = max(ws.max_row or 1, len(statuses) + 5)
    old_c = max(ws.max_column or 1, len(col_dates) + 5, 30)
...
def generate_report(tc_file, shopee_file, wms_files, inv_file,
    ...
    # Deduplicate by invoice_number — TC has multiple rows per invoice (one per SKU)
    df_filtered = filtered[
        ~filtered["WMS"].isin({"NOT FOUND"}) &
        ~is_cancel &
        ~is_zero_stock
    ].drop_duplicates(subset=["invoice_number"]).copy()

    df_not_pushed = filtered[filtered["WMS"] == "NOT FOUND"].drop_duplicates(subset=["invoice_number"]).copy()
    ...
    for f in tiktok_files:
        try:
            ...
            # Detect format: full export has many columns; old export is single-col OrderSKUList
            xl = pd.ExcelFile(BytesIO(raw_bytes))
            if "OrderSKUList" in xl.sheet_names:
                # Old format: single column of Order IDs
                ...
                for oid in order_ids[~order_ids.isin(tc_order_ids)]:
                    mp_missing_rows.append({
                        "Order Number": oid, "Marketplace": "TikTok",
                        "Store": store_name, "MP Status": "",
                        "Ordered Date": "", "MP SLA": "",
                        "Item Name": "", "Seller SKU": "",
                    })
            else:
                # New full export: row 0 = headers, row 1 = descriptions (skip)
                ...
                for _, tr in tt_missing.iterrows():
                    ...
                    mp_missing_rows.append({
                        "Order Number": oid, "Marketplace": "TikTok",
                        "Store":        store_name, "MP Status": mp_status,
                        "Ordered Date": ordered_date, "MP SLA": mp_sla,
...
def _read_mp_lookup(shopee_bytes, tiktok_bytes_list, lazada_bytes):
    ...
    for tt_bytes, store in (tiktok_bytes_list or []):
        try:
            xl = pd.ExcelFile(BytesIO(tt_bytes))
            if "OrderSKUList" in xl.sheet_names:
                # Old format: single-column Order IDs only
                raw = pd.read_excel(BytesIO(tt_bytes), sheet_name="OrderSKUList",
                                    header=None, dtype=str)
                order_ids = raw.iloc[2:, 0].dropna().astype(str).str.strip()
                order_ids = order_ids[order_ids != "Platform unique order ID."]
                for oid in order_ids:
                    if oid not in lookup:
                        lookup[oid] = {
                            "payment_status": "COMPLETED", "order_item_status": "READY TO SHIP",
                            "ordered_date": "", "nickname": store, "mp_sla": "",
                            "mp_status": "", "item_name": "", "seller_sku": "",
                        }
            else:
                # New full export: row 0 = headers, row 1 = descriptions (skip)
                ...
                for _, r in tt.iterrows():
                    ...
                    if not oid or oid == "nan":
                        continue
                    mp_status    = str(r.get("Order Status", "")).strip() if "Order Status" in tt.columns else ""
                    pay_status   = "PENDING" if mp_status.lower() == "unpaid" else "COMPLETED"
                    ordered_date = str(r.get("Created Time", "")).strip() if "Created Time" in tt.columns else ""
                    mp_sla       = add_one_day(ordered_date)
                    lookup[oid] = {
                        "payment_status":    pay_status,
                        "order_item_status": "READY TO SHIP",
    ...
    if not all([tc_file, shopee_file, wms_files, inv_file]):
        ...
        with st.spinner("Processing files…"):
            try:
                ...
                st.session_state["report_last_updated"] = None
...
if st.session_state.get("report_bytes"):
    ...
    # ── Persistent download — always visible after any update ─────────────────
    if st.session_state.get("report_last_updated"):
        st.markdown("---")
        st.success(f"✅ Report updated — {st.session_state['report_last_updated']}")
        st.download_button(
                ...
                if do_update:
                    ...
                    with st.spinner(f"Updating {inv_num}…"):
                        try:
                            ...
                            st.session_state["report_last_updated"] = f"Invoice {inv_num} → FILTERED DATA ({new_wms})"
                        except Exception as e:
                            st.error(f"Error: {e}")
    ...
    if nitc_rows:
        ...
        st.caption("Enter Invoice Number + WMS status. Other details auto-filled from MP files where available — fill in manually if blank.")
        ...
        for idx, row in enumerate(nitc_rows):
            ...
            has_mp     = bool(mp_info.get("ordered_date") or mp_info.get("mp_sla"))

            _item_raw  = row.get("Item Name") or row.get("item_name") or mp_info.get("item_name") or "—"
            item_label = str(_item_raw)[:40]

            # Resolved values — prefer sheet data, fall back to mp_lookup
            res_ordered  = str(row.get("Ordered Date") or mp_info.get("ordered_date") or "")
            res_sla      = str(row.get("MP SLA")       or mp_info.get("mp_sla")       or "")
            res_status   = str(row.get("MP Status")    or mp_info.get("mp_status")    or "")
            res_nickname = str(mp_info.get("nickname") or row.get("Store")            or "")
            res_pay      = str(mp_info.get("payment_status")    or "COMPLETED")
            res_ois      = str(mp_info.get("order_item_status") or "READY TO SHIP")
            res_item     = str(row.get("Item Name")  or mp_info.get("item_name")  or "—")
            res_sku      = str(row.get("Seller SKU") or mp_info.get("seller_sku") or "—")
            ...
            with st.expander(
                ...
                if not has_mp:
                    st.info("ℹ️ MP file data not available for this order (old TikTok export format — Order ID only). Please fill in the details manually below.")

                c_left, c_right, c_btn = st.columns([2, 2, 1])

                with c_left:
                    st.markdown(f"**Order Number:** `{order_num}`")
                    st.markdown(f"**Marketplace:** {row.get('Marketplace','')}")
                    st.markdown(f"**MP Status:** {res_status or '—'}")
                    st.markdown(f"**Item:** {res_item[:60]}")
                    st.markdown(f"**SKU:** {res_sku}")

                with c_right:
                    inv_input = st.text_input(
                        "Invoice Number *", key=f"nitc_inv_{idx}",
                        placeholder="e.g. GSK00364795")
                    wms_input = st.selectbox(
                        "WMS Status",
                        ["PENDING_FULFILLMENT", "FULFILLED", "PARTIAL_ALLOCATED", "NOT FOUND"],
                        key=f"nitc_wms_{idx}")

                    # Editable fields — pre-filled from MP data if available
                    nickname_input = st.text_input(
                        "Nickname", value=res_nickname,
                        key=f"nitc_nick_{idx}",
                        placeholder="e.g. tiktok-Haleon")
                    ordered_date_input = st.text_input(
                        "Ordered Date", value=res_ordered,
                        key=f"nitc_date_{idx}",
                        placeholder="e.g. 01/07/2026 10:00:00")
                    mp_sla_input = st.text_input(
                        "MP SLA (YYYY-MM-DD)", value=res_sla,
                        key=f"nitc_sla_{idx}",
                        placeholder="e.g. 2026-07-02")
                    pay_input = st.selectbox(
                        "Payment Status", ["COMPLETED", "PENDING"],
                        index=0 if res_pay == "COMPLETED" else 1,
                        key=f"nitc_pay_{idx}")
                ...
                if do_nitc_update:
                    if not inv_input.strip():
                        ...
                        # Use manually entered values (already pre-filled from MP where available)
                        _sla = mp_sla_input.strip() or add_one_day(ordered_date_input.strip())
                        new_row = {
                            "Order Number":      order_num,
                            "Invoice Number":    inv_input.strip(),
                            "Payment Status":    pay_input,
                            "Order Item Status": res_ois,
                            "Ordered Date":      ordered_date_input.strip(),
                            "Nickname":          nickname_input.strip() or res_nickname,
                            "MP SLA":            _sla,
                            "WMS":               wms_input,
                        }
                        with st.spinner(f"Updating {order_num}…"):
                            try:
                                ...
                                st.session_state["report_last_updated"] = f"Order {order_num} (Invoice {inv_input.strip()}) → FILTERED DATA ({wms_input})"
                            except Exception as e:
                                st.error(f"Error: {e}")
