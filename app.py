...
def generate_report(tc_file, shopee_file, wms_files, inv_file,
    ...
    # ── TikTok files ──────────────────────────────────────────────────────────
    TIKTOK_PENDING = {"AWAITING_SHIPMENT", "READY_TO_SHIP", "PROCESSING",
                      "AWAITING COLLECTION", "ON_HOLD"}
    for f in tiktok_files:
        try:
            raw_bytes = f.read() if hasattr(f, "read") else f
            raw = pd.read_excel(BytesIO(raw_bytes), sheet_name="OrderSKUList",
                                header=None, dtype=str)
            # Row 0 = "Order ID", Row 1 = description, Row 2+ = actual IDs
            store_name = getattr(f, "name", "TikTok").replace(".xlsx","")
            order_ids = raw.iloc[2:, 0].dropna().astype(str).str.strip()
            order_ids = order_ids[order_ids != "Platform unique order ID."]
            missing = order_ids[~order_ids.isin(tc_order_ids)]
            for oid in missing:
                mp_missing_rows.append({
                    "Order Number": oid,
                    "Marketplace":  "TikTok",
        ...
        try:
            ...
            for _, lr in laz_missing_dedup.iterrows():
                mp_missing_rows.append({
                    ...
                    "Ordered Date": lr.get("createTime", ""),
                    "MP SLA":       lr.get("rtsSla", ""),
...
tab1, tab2, tab3 = st.tabs([
    "🗂️ Generate Report",
    "📦 Orders Not Pushed to WMS",
    "🔄 Update NOT IN TC Orders",
])
...
with tab1:
    ...
    with col_c:
        tiktok_files = st.file_uploader(
            "TikTok Order File(s) (.xlsx) — upload both stores",
            ...
            with st.spinner("Processing files…"):
                try:
                    wb, summary = generate_report(
                        tc_file, shopee_file, wms_files, inv_file,
                        tiktok_files=tiktok_files or [],
                        lazada_file=lazada_file,
                    )
                    buf = BytesIO(); wb.save(buf); report_bytes = buf.getvalue()
                    ...
                    # ── Preview: NOT PUSHED TO WMS ───────────────────────────
                    if summary.get("NOT PUSHED TO WMS", 0) > 0:
                        st.markdown("---")
                        st.markdown(f"#### 🔴 Not Pushed to WMS — {summary['NOT PUSHED TO WMS']} order(s)")
                        st.info("👇 Copy Order Numbers below to check if already pushed in WMS, then go to **Orders Not Pushed to WMS** tab to update.")
                        from openpyxl import load_workbook as _lw
                        _wb = _lw(BytesIO(report_bytes), read_only=True)
                        _ws = _wb["NOT PUSHED TO WMS"]
                        _headers = [_ws.cell(1, c).value for c in range(1, 9)]
                        _rows = []
                        for _r in range(2, _ws.max_row + 1):
                            _rv = [_ws.cell(_r, c).value for c in range(1, 9)]
                            if any(v is not None for v in _rv):
                                _rows.append(_rv)
                        if _rows:
                            _preview_df = pd.DataFrame(_rows, columns=_headers)
                            st.dataframe(_preview_df, use_container_width=True, hide_index=True)

                    # ── Preview: NOT IN TC ────────────────────────────────────
                    if summary.get("NOT IN TC", 0) > 0:
                        st.markdown("---")
                        st.markdown(f"#### 🟡 Not in TC — {summary['NOT IN TC']} order(s)")
                        st.info("👇 Copy Order Numbers below to check if already pushed in TC, then go to **Update NOT IN TC Orders** tab to update.")
                        from openpyxl import load_workbook as _lw2
                        _wb2 = _lw2(BytesIO(report_bytes), read_only=True)
                        _ws2 = _wb2["NOT IN TC"]
                        _headers2 = [_ws2.cell(1, c).value for c in range(1, _ws2.max_column + 1)]
                        _rows2 = []
                        for _r in range(2, _ws2.max_row + 1):
                            _rv2 = [_ws2.cell(_r, c).value for c in range(1, _ws2.max_column + 1)]
                            if any(v is not None for v in _rv2):
                                _rows2.append(_rv2)
                        if _rows2:
                            _preview_df2 = pd.DataFrame(_rows2, columns=_headers2)
                            st.dataframe(_preview_df2, use_container_width=True, hide_index=True)
                except Exception as e:
                    st.error(f"Error: {e}"); st.exception(e)


# ─────────────────────────────────────────────────────────────────────────────
#  TAB 2 — ORDERS NOT PUSHED TO WMS
# ─────────────────────────────────────────────────────────────────────────────
with tab2:
    st.subheader("Orders confirmed as pushed to WMS")
    existing2 = st.file_uploader(
        "Upload existing report (.xlsx)  *(optional if already generated above)*",
        type=["xlsx"], key="existing_report",
    )
    if existing2:
        st.session_state["report_bytes"]   = existing2.read()
        st.session_state["report_filename"] = existing2.name

    has_report2 = bool(st.session_state.get("report_bytes"))
    if has_report2:
    ...
    st.markdown("---")
    st.markdown("#### Paste invoice numbers that are now in WMS\nOne per line, or comma-separated")
    invoice_text = st.text_area("Invoice Numbers", height=160, key="invoice_input",
                                 placeholder="GSK00352711\nGSK00352812\nGSK00353001")
    wms_status = st.selectbox("WMS status to assign",
                               ["PENDING_FULFILLMENT", "FULFILLED", "PARTIAL_ALLOCATED"],
                               key="wms_status_select")

    if st.button("✅ Update Report", type="primary",
                 use_container_width=True, disabled=not has_report2):
        ...
        if not raw:
            ...
            with st.spinner(f"Updating {len(invoices)} invoice(s)…"):
                try:
                    ...
                    if matched:
                        st.success(f"✅ **{matched} invoice(s)** moved to FILTERED DATA with `{wms_status}`. PIVOT rebuilt.")
                    else:
                        st.warning("None found in NOT PUSHED TO WMS sheet.")
                    if not_found:
                        st.warning(f"⚠️ Not found: {', '.join(sorted(not_found))}")
                    st.download_button("⬇️ Download Updated Report", data=updated_bytes,
...
# ─────────────────────────────────────────────────────────────────────────────
#  TAB 3 — UPDATE NOT IN TC ORDERS
# ─────────────────────────────────────────────────────────────────────────────
with tab3:
    st.subheader("Update orders found in NOT IN TC tab")

    existing3 = st.file_uploader(
        "Upload existing report (.xlsx)  *(optional if already generated above)*",
        type=["xlsx"], key="existing_report_t3",
    )
    if existing3:
        st.session_state["report_bytes"]   = existing3.read()
        st.session_state["report_filename"] = existing3.name

    has_report3 = bool(st.session_state.get("report_bytes"))
    if has_report3:
        st.success(f"📄 Loaded: **{st.session_state.get('report_filename', 'report.xlsx')}**")
    else:
        st.warning("No report loaded. Generate one in Tab 1 or upload an existing file above.")
    ...
    # ── Option A: Re-upload fresh TC + WMS ───────────────────────────────────
    st.markdown("### Option A — Re-upload updated TC & WMS files")
    st.markdown(
        "Upload fresh exports. Orders that now appear in TC will be removed from "
        "NOT IN TC and added to FILTERED DATA automatically."
    )
    col_r1, col_r2 = st.columns(2)
    with col_r1:
        new_tc_file = st.file_uploader("Updated TC Order Report (.csv)", type=["csv"], key="refresh_tc")
    with col_r2:
        new_wms_files = st.file_uploader("Updated WMS File(s) (.xlsx)", type=["xlsx"],
                                          key="refresh_wms", accept_multiple_files=True)

    if st.button("🔄 Refresh from Files", type="primary",
                 use_container_width=True, disabled=not has_report3):
        if not new_tc_file or not new_wms_files:
            st.error("Please upload both an updated TC file and at least one WMS file.")
        else:
            with st.spinner("Re-checking NOT IN TC against updated files…"):
                try:
                    updated_bytes, resolved = refresh_not_in_tc(
                        st.session_state["report_bytes"], new_tc_file, new_wms_files)
                    st.session_state["report_bytes"] = updated_bytes
                    if resolved:
                        st.success(f"✅ **{resolved} order(s)** now in TC — moved to FILTERED DATA. PIVOT rebuilt.")
                    else:
                        st.info("No orders from NOT IN TC were found in the updated TC file.")
                    st.download_button("⬇️ Download Updated Report", data=updated_bytes,
                                        file_name=st.session_state.get("report_filename", "GSK_Report_Updated.xlsx"),
                                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                        use_container_width=True)
    ...
    # ── Option B: Multi-row manual entry via editable table ──────────────────
    st.markdown("### Option B — Enter order details manually")
    st.markdown(
        "Fill in as many rows as needed below — same columns as FILTERED DATA. "
        "All non-empty rows will be added to FILTERED DATA and removed from NOT IN TC."
    )

    # Default empty grid — 5 blank rows to start
    ...
    edited_df = st.data_editor(
        ...
        use_container_width=True,
        key="nitc_manual_grid",
        column_config={
            "Payment Status": st.column_config.SelectboxColumn(
                options=["COMPLETED", "PENDING"], required=False),
            "Order Item Status": st.column_config.SelectboxColumn(
                options=["READY TO SHIP", "ACCEPTED/PICKED", "NEW"], required=False),
            "Nickname": st.column_config.SelectboxColumn(
                options=["shopee-SG", "tiktok-Haleon", "tiktok-Caltrate",
                         "lazada-SG", "offline-SG"], required=False),
            "WMS": st.column_config.SelectboxColumn(
                options=["PENDING_FULFILLMENT", "FULFILLED",
                         "PARTIAL_ALLOCATED", "NOT FOUND"], required=False),
            "MP SLA":       st.column_config.TextColumn(help="Format: YYYY-MM-DD"),
            "Ordered Date": st.column_config.TextColumn(help="e.g. 01/07/2026 10:00:00"),
        },
    )

    if st.button("➕ Add Rows to FILTERED DATA", type="primary",
                 use_container_width=True, disabled=not has_report3):
        # Only process rows where Order Number is filled
        valid_rows = edited_df[edited_df["Order Number"].str.strip().ne("")]
        if valid_rows.empty:
            st.error("Please fill in at least one Order Number.")
        else:
            with st.spinner(f"Adding {len(valid_rows)} order(s)…"):
                try:
                    current_bytes = st.session_state["report_bytes"]
                    for _, row in valid_rows.iterrows():
                        new_row = {
                            "Order Number":      str(row["Order Number"]).strip(),
                            "Invoice Number":    str(row["Invoice Number"]).strip(),
                            "Payment Status":    str(row["Payment Status"]).strip(),
                            "Order Item Status": str(row["Order Item Status"]).strip(),
                            "Ordered Date":      str(row["Ordered Date"]).strip(),
                            "Nickname":          str(row["Nickname"]).strip(),
                            "MP SLA":            str(row["MP SLA"]).strip(),
                            "WMS":               str(row["WMS"]).strip(),
                        }
                        current_bytes = update_not_in_tc_manual(current_bytes, new_row)
                    st.session_state["report_bytes"] = current_bytes
                    st.success(
                        f"✅ **{len(valid_rows)} order(s)** added to FILTERED DATA "
                        "and removed from NOT IN TC. PIVOT rebuilt."
                    )
                    st.download_button(
                        "⬇️ Download Updated Report", data=current_bytes,
                        file_name=st.session_state.get("report_filename", "GSK_Report_Updated.xlsx"),
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        use_container_width=True,
                    )
