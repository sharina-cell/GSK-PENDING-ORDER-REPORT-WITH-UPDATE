...
with tab3:
    ...
    st.markdown(
        "Fill in the order details below. The order will be added to FILTERED DATA "
        "and removed from NOT IN TC."
    )

    col_m1, col_m2, col_m3 = st.columns(3)
    with col_m1:
        m_order_num   = st.text_input("Order Number",   key="m_order_num")
        m_inv_num     = st.text_input("Invoice Number", key="m_inv_num")
        m_pay_status  = st.selectbox("Payment Status", ["COMPLETED", "PENDING"], key="m_pay_status")
    with col_m2:
        m_item_status  = st.selectbox("Order Item Status",
                                       ["READY TO SHIP", "ACCEPTED/PICKED", "NEW"],
                                       key="m_item_status")
        m_ordered_date = st.text_input("Ordered Date  (e.g. 01/07/2026 10:00:00)", key="m_ordered_date")
        m_nickname     = st.selectbox("Nickname",
                                       ["shopee-SG", "tiktok-Haleon", "tiktok-Caltrate",
                                        "lazada-SG", "offline-SG"],
                                       key="m_nickname")
    with col_m3:
        m_mp_sla = st.text_input("MP SLA  (YYYY-MM-DD)", key="m_mp_sla")
        m_wms    = st.selectbox("WMS Status",
                                 ["PENDING_FULFILLMENT", "FULFILLED", "PARTIAL_ALLOCATED", "NOT FOUND"],
                                 key="m_wms")

    if st.button("➕ Add to FILTERED DATA", type="primary",
                 use_container_width=True, disabled=not has_report3):
        if not m_order_num.strip():
            st.error("Order Number is required.")
        else:
            with st.spinner("Adding order…"):
                try:
                    new_row = {
                        "Order Number":      m_order_num.strip(),
                        "Invoice Number":    m_inv_num.strip(),
                        "Payment Status":    m_pay_status,
                        "Order Item Status": m_item_status,
                        "Ordered Date":      m_ordered_date.strip(),
                        "Nickname":          m_nickname,
                        "MP SLA":            m_mp_sla.strip(),
                        "WMS":               m_wms,
                    }
                    updated_bytes = update_not_in_tc_manual(
                        st.session_state["report_bytes"], new_row)
                    st.session_state["report_bytes"] = updated_bytes
                    st.success(f"✅ Order **{m_order_num.strip()}** added to FILTERED DATA and removed from NOT IN TC.")
                    st.download_button("⬇️ Download Updated Report", data=updated_bytes,
                                        file_name=st.session_state.get("report_filename", "GSK_Report_Updated.xlsx"),
                                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                        use_container_width=True)
