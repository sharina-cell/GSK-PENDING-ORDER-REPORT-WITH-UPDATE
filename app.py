        ...
        if not all([tc_file, shopee_file, wms_files, inv_file]):
            ...
            with st.spinner("Processing files…"):
                try:
                    ...
                    if summary.get("NOT PUSHED", 0) > 0:
                        st.info(f"💡 **{summary['NOT PUSHED']} order(s) NOT PUSHED** — go to the **Update NOT PUSHED Orders** tab.")
                    if summary.get("NOT IN TC", 0) > 0:
                        st.info(f"💡 **{summary['NOT IN TC']} order(s) NOT IN TC** — go to the **Update NOT IN TC Orders** tab.")
                except Exception as e:
            ...
            with st.spinner(f"Updating {len(invoices)} invoice(s)…"):
                try:
                    ...
                    if matched:
                        ...
                        st.warning("None found in NOT PUSHED sheet.")
                    if not_found:
