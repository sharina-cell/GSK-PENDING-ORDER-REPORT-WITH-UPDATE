...
# ── TikTok XML parser ─────────────────────────────────────────────────────────
def _parse_tiktok_xml(file_bytes):
    """
    Parse TikTok order XLSX where all rows share r='1' in the XML —
    reconstruct full data from cell references.
    Row 1 = headers, Row 2 = subheader descriptions (skip), Row 3+ = data.
    ...
    with zipfile.ZipFile(buf) as z:
        ...
        if not sheet_files:
            return []
        xml = z.read(sheet_files[0]).decode('utf-8', errors='replace')
        shared_strings = []
        if 'xl/sharedStrings.xml' in z.namelist():
            ss_xml = z.read('xl/sharedStrings.xml').decode('utf-8', errors='replace')
            shared_strings = _re.findall(r'<t[^>]*>(.*?)</t>', ss_xml)
    ...
    cells = {}
    for m in _re.finditer(r'<c r="([A-Z]+)(\d+)"([^>]*)>(.*?)</c>', xml, _re.DOTALL):
        col_str, row_str, attrs, content = m.groups()
        row_num = int(row_str)
        col_num = _col_num(col_str)
        t_m = _re.search(r't="([^"]*)"', attrs)
        ctype = t_m.group(1) if t_m else ''
        if ctype == 's':
            v_m = _re.search(r'<v>(\d+)</v>', content)
            val = shared_strings[int(v_m.group(1))] if v_m and int(v_m.group(1)) < len(shared_strings) else ''
        elif ctype == 'inlineStr':
            v_m = _re.search(r'<t[^>]*>(.*?)</t>', content)
    ...
    return [
        {headers[c - 1]: cells.get((r, c), '') for c in range(1, max_col + 1)}
        for r in range(3, max_row + 1)   # row 2 is subheader — skip
    ]


# ── MP file lookup builder ────────────────────────────────────────────────────
def _read_mp_lookup(shopee_bytes, tiktok_bytes_list, lazada_bytes):
    """
    Build lookup dict  order_id -> {payment_status, order_item_status,
        ...
        try:
            ...
            for _, r in sh.iterrows():
                ...
                lookup[oid] = {
                    ...
                    "nickname":          "shopee-SG",
                    "mp_sla":            sla[:10] if sla and sla != "nan" else "",
                    "mp_status":         mp_status,
    ...
    # TikTok — use XML parser to handle the r="1" bug
    for tt_bytes, store in (tiktok_bytes_list or []):
        try:
            for r in _parse_tiktok_xml(tt_bytes):
                oid = str(r.get("Order ID", "")).strip()
                if not oid or oid == "nan":
                    continue
                mp_status    = str(r.get("Order Status", "")).strip()
...
def generate_report(tc_file, shopee_file, wms_files, inv_file,
    ...
    # ── TikTok files ──────────────────────────────────────────────────────────
    TIKTOK_PENDING = {"to ship", "awaiting shipment", "awaiting collection",
                      "ready to ship", "processing", "on hold"}
    for f in tiktok_files:
        try:
            raw_bytes  = f.read() if hasattr(f, "read") else f
            store_name = getattr(f, "name", "TikTok").replace(".xlsx", "")
            tt_rows    = _parse_tiktok_xml(raw_bytes)
            for tr in tt_rows:
                oid = str(tr.get("Order ID", "")).strip()
                if not oid or oid == "nan":
                    continue
                mp_status = str(tr.get("Order Status", "")).strip()
                if mp_status.lower() not in TIKTOK_PENDING:
                    continue
                if oid in tc_order_ids:
                    continue
